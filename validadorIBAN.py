#El Validador IBAN
#El cuarto programa implementa (en una forma ligeramente simplificada) un algoritmo utilizado por 
#los bancos europeos para especificar los números de cuenta. El estándar llamado IBAN (Número de cuenta bancaria internacional) 
...
#Un código de país de dos letras tomado del estándar ISO 3166-1 (por ejemplo, FR para Francia, GB para Gran Bretaña DE para Alemania, 
#y así sucesivamente).
#Dos dígitos de verificación utilizados para realizar las verificaciones de validez: pruebas rápidas y simples, pero no totalmente confiables, 
...
#(Paso 1) Verificar que la longitud total del IBAN sea correcta según el país (este programa no lo hará, pero puedes modificar el 
#código para cumplir con este requisito si lo deseas; nota: pero debes enseñar al código a conocer todas las longitudes utilizadas en Europa).
...
#(Paso 4) Interpreta la cadena como un entero decimal y calcula el residuo de ese número dividiendolo entre 97. Si el residuo es 1, 
#pasa la prueba de verificación de dígitos y el IBAN puede ser válido.
#Observa el código en el editor. Analicémoslo:
# Validador IBAN

iban = input("Ingresa IBAN, por favor: ")
iban = iban.replace(' ','') #Espacios se reemplazan por no espacios
if not iban.isalnum():
    ...
    iban = (iban[4:] + iban[0:4]).upper() #se mueven los cuatro caracteres iniciales al final del número  (nota cómo juega con los cortes 
    #de lista)
    #y se convierten todas las letras a mayúsculas (paso 02 del algoritmo).
    iban2 = '' # esta es la variable utilizada para completar el número, creada al reemplazar las letras con dígitos 
    #(de acuerdo con el paso 03 del algoritmo).
    for ch in iban:
        if ch.isdigit():
            iban2 += ch
        else:
...
#Línea 13: esta es la variable utilizada para completar el número, creada al reemplazar las letras con dígitos 
#(de acuerdo con el paso 03 del algoritmo).
